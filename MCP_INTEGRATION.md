...
**OAuth metadata:**
- Authorization server metadata: `/.well-known/oauth-authorization-server` (generated with the MCP SDK's
  `createOAuthMetadata` helper to match `mcpAuthRouter` defaults)
- Protected resource metadata: `/.well-known/oauth-protected-resource`

**Primary endpoints:**
- `GET /authorize`
- `POST /token` (requires `client_secret_post` for registered clients)
- `POST /register` (dynamic client registration; server-issued `client_id` and `client_secret`)
